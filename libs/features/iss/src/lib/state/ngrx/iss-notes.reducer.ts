...
export const reducer = createReducer(
  initialState,
  on(IssNotesActions.loadWeekRequested, (s) => ({ ...s, loading: true, error: null })),
  on(IssNotesActions.loadWeekSucceeded, (s, { notes }) =>
    adapter.setAll(notes, { ...s, loading: false, error: null })
  ),
  on(IssNotesActions.loadWeekFailed, (s, { error }) => ({ ...s, loading: false, error })),
  ...
  on(IssNotesActions.createFailed, (s, { error }) => ({ ...s, loading: false, error })),

  on(IssNotesActions.updateSucceeded, (s, { note }) =>
    adapter.upsertOne(note, s)
  ),
  on(IssNotesActions.removeSucceeded, (s, { id }) =>
    adapter.removeOne(id, s)
  ),
  on(IssNotesActions.clearLocal, (s) =>
    adapter.removeAll(s)
  ),

  on(IssNotesActions.setWeekAnchor, (s, { anchor }) => ({ ...s, weekAnchor: anchor })),
  on(IssNotesActions.setFilterConsumer, (s, { consumer }) => ({ ...s, filterConsumer: consumer ?? '' })),
  on(IssNotesActions.setFilterLocation, (s, { location }) => ({ ...s, filterLocation: location ?? '' })),
  on(IssNotesActions.setFilterQuery,    (s, { q }) => ({ ...s, filterQuery: q ?? '' })),
);
...
export const IssNotesFeature = createFeature({
  ...
  extraSelectors: ({ selectIssNotesState }) => {
    const { selectAll, selectTotal, selectEntities, selectIds } = adapter.getSelectors(selectIssNotesState);
    return {
      selectAll,
      selectTotal,
      selectEntities,
      ...
      selectWeekListFiltered: (root: any) => {
        ...
        return list
          .filter(n => {
            ...
            const inC = !c || `${n.consumer ?? ''}`.toLowerCase().includes(c);
            const inL = !l || `${n.location ?? ''}`.toLowerCase().includes(l);
            return inQ && inC && inL;
          })
